...
<body>
    ...
    if (isset($_GET["action"])) {
        if (file_exists("Guest.txt") || filesize("Guest.txt") !== 0) {
            ...
            switch ($_GET["action"]) {
                ...
                echo "There are remaining Name on the list.<br>";
                //debug
            } else {
                unlink("Guest.txt");
            }
    ...
    if (!file_exists("Guest.txt") || filesize("Guest.txt") == 0) {
        ...
        foreach ($keyMessageArray as $message) {
            ...
            echo "<span style=\"font-weight: bold\">Email: </span>" . htmlentities($currMsg[0]) . "<br>\n";
            echo "<span style=\"text-decoration: underline; font-weight: bold;\">Feedback: </span><br>\n" . htmlentities($currMsg[1]) . "</td>\n";
